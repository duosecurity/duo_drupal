...
class Duo {
    ...
    public static function signRequest($ikey, $skey, $akey, $username, $time=NULL) {
        if (!isset($username) || drupal_strlen($username) === 0) {
            return self::ERR_USER;
        }
        if (strpos($username, '|') !== FALSE) {
            return self::ERR_USER;
        }
        if (!isset($ikey) || drupal_strlen($ikey) !== self::IKEY_LEN) {
            return self::ERR_IKEY;
        }
        if (!isset($skey) || drupal_strlen($skey) !== self::SKEY_LEN) {
            return self::ERR_SKEY;
        }
        if (!isset($akey) || drupal_strlen($akey) < self::AKEY_LEN) {
